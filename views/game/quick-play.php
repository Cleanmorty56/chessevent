...
$this->title = 'Быстрая игра в шахматы';
...
.alert-info { background: #e0f7fa; color: #006064; border: 2px solid #006064; }
...
.bg-secondary { background: #666 !important; }
    ...
    <div class="quick-play">
        ...
        <div class="game-container">
            <div class="room-info">
                ...
                <div class="room-code" id="room-code">Подключение...</div>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="player-card you">
                        ...
                        <p><strong>Имя:</strong> <?= Html::encode(isset($user->username) ? $user->username : 'Гость') ?></p>
                    ...
                    <div class="game-info">
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <button id="copy-room-btn" class="btn btn-info control-btn" disabled>
                                <i class="fas fa-user-plus"></i> Пригласить друга
                            </button>
            ...
            <div class="controls mt-4">
                <p class="text-muted mb-3">
                    <i class="fas fa-info-circle"></i> Чтобы начать игру, нажмите "Найти игру" или пригласите друга
                </p>
...
<?php
$jsUserId = isset($userId) ? $userId : 0;
$jsUsername = isset($user->username) ? addslashes($user->username) : 'Гость';
...
window.findNewOpponent = function() {
    $('.game-over-modal').remove(); gameEnded = false;
    if (ws && ws.readyState === WebSocket.OPEN && roomId) { ws.send(JSON.stringify({type: 'leave_room', roomId: roomId})); }
    resetGame();
...
function initChessBoard() {
    try {
        board = Chessboard('board', {
            position: 'start', orientation: 'white', draggable: false,
            dropOffBoard: 'snapback', onDragStart: function(){ return false; },
...
function handleSquareClick(square) {
    ...
    if (piece && pieceColor === myPieceColor) {
        ...
        moves.forEach(function(move) {
            ...
            if (targetPiece) { $('.square-55d63[data-square="' + targetSquare + '"]').addClass('capture-square'); }
            else { $('.square-55d63[data-square="' + targetSquare + '"]').addClass('highlight-square'); }
        });
    } else if (selectedSquare) {
        var targetSquare = square;
        var validMove = possibleMoves.find(function(move) { return move.to === targetSquare; });
        if (validMove) { executeMove(selectedSquare, targetSquare); }
        selectedSquare = null; possibleMoves = []; clearHighlights();
    }
}
...
function executeMove(from, to) {
    try {
        ...
        board.position(game.fen()); isMyTurn = false; updateTurnIndicator();
        if (ws && ws.readyState === WebSocket.OPEN && roomId) {
            ws.send(JSON.stringify({ type: 'move', roomId: roomId, move: move.san, fen: game.fen() }));
        }
        addMoveToHistory(move.san); checkGameEnd(); return true;
    } catch (e) { return false; }
}

function clearHighlights() { $('.square-55d63').removeClass('selected-square highlight-square capture-square'); }
...
function connectWebSocket() {
    try {
        ...
        ws.onmessage = function(event) {
            try {
                ...
                switch(data.type) {
                    ...
                    case 'draw_offered': handleDrawOffered(data); break;
                    case 'error': break;
                }
        ...
        ws.onclose = function(event) {
            ...
            } else { $('#new-game-btn').prop('disabled', false); }
        };
        ws.onerror = function(error) { console.error('WebSocket ошибка:', error); };
    } catch (e) { console.error('Ошибка WebSocket:', e); }
}

function handleWaiting(data) {
    $('#room-code').text('Поиск игры...');
    $('#turn-indicator').html('<i class="fas fa-search"></i> Поиск соперника...').removeClass().addClass('alert alert-info');
    $('#new-game-btn').prop('disabled', true); $('#manual-start-btn').hide();
    gameInProgress = false; gameEnded = false; clearHighlights(); $('.game-over-modal').remove();
}

function handleRoomAssigned(data) {
    roomId = data.roomId;
    $('#room-code').text('Подключено');
    $('#copy-room-btn').prop('disabled', false);
    $('#manual-start-btn').show();
}

function handlePlayerJoined(data) {
    opponent = data.player; updateOpponentInfo();
...
function handleGameStart(data) {
    myColor = data.yourColor; opponent = data.opponent;
    gameInProgress = true; gameEnded = false;
    updatePlayerInfo(); updateOpponentInfo(); game.reset();
    if (board) { board.position('start'); board.orientation(myColor); isMyTurn = (myColor === 'white'); }
    updateTurnIndicator();
    $('#resign-btn, #offer-draw-btn').prop('disabled', false);
    $('#chat-input, #send-chat').prop('disabled', false);
    $('#new-game-btn').prop('disabled', true);
    $('#copy-room-btn').prop('disabled', true);
    $('#manual-start-btn').hide();
    $('#move-history').html(''); clearHighlights(); $('.game-over-modal').remove();
    if (isMyTurn) {
...
function handleOpponentMove(data) {
    ...
    try {
        ...
        if (move) {
            ...
            addMoveToHistory(move.san); isMyTurn = true; updateTurnIndicator(); checkGameEnd(); clearHighlights();
        } else if (data.move && data.move.length >= 4) {
            var moveUCI = game.move({ from: data.move.substring(0, 2), to: data.move.substring(2, 4), promotion: 'q' });
            if (moveUCI) {
                if (board) board.position(game.fen());
                addMoveToHistory(moveUCI.san); isMyTurn = true; updateTurnIndicator(); checkGameEnd(); clearHighlights();
            }
        }
    } catch (e) { console.error('Ошибка хода:', e); }
}

function handlePlayerLeft(data) {
    resetGame();
    $('#opponent-name').text('Ожидание...'); $('#opponent-color').text('ожидание...');
    ...
    $('#resign-btn, #offer-draw-btn, #chat-input, #send-chat').prop('disabled', true);
    $('#new-game-btn').prop('disabled', false); $('#copy-room-btn').prop('disabled', false);
    opponent = null; myColor = null; gameInProgress = false; gameEnded = false;
    clearHighlights(); $('.game-over-modal').remove();
    setTimeout(function() { showGameOverModal('Соперник покинул игру', 'player_left'); }, 500);
}

function handleGameOver(data) {
    if (gameEnded) return;
    gameInProgress = false; gameEnded = true;
    var message = '';
    var eloText = '';
    if (data.elo) {
        var myChange = myColor === 'white' ? data.elo.white_change : data.elo.black_change;
        var sign = myChange > 0 ? '+' : '';
        eloText = ' (ELO: ' + sign + myChange + ')';
    }
    switch(data.result) {
        case 'white_win': message = myColor === 'white' ? '🏆 Победа!' : '💔 Поражение'; break;
        case 'black_win': message = myColor === 'black' ? '🏆 Победа!' : '💔 Поражение'; break;
        case 'draw': message = '🤝 Ничья!'; break;
    }
    message += eloText;
    ...
    setTimeout(function() { showGameOverModal(message, data.result); }, 500);
}

function handleDrawOffered(data) {
    if (confirm(data.fromUsername + ' предлагает ничью. Принять?')) {
        if (ws && ws.readyState === WebSocket.OPEN) { ws.send(JSON.stringify({ type: 'accept_draw', roomId: roomId })); }
    } else {
        if (ws && ws.readyState === WebSocket.OPEN) { ws.send(JSON.stringify({ type: 'reject_draw', roomId: roomId })); }
    }
}
...
function addMoveToHistory(move) {
    ...
    if (history.length % 2 === 1) {
        ...
        if (lastRow.length) { lastRow.append(' <span class="black-move">' + move + '</span>'); }
        else { $('#move-history').append('<div class="move-row"><strong>' + moveNumber + '.</strong> ... <span class="black-move">' + move + '</span></div>'); }
    }
...
function endGame(result, message) {
    ...
    roomId = null; $('#room-code').text('Подключение...');
    isMyTurn = false;
    $('#resign-btn, #offer-draw-btn').prop('disabled', true);
    $('#new-game-btn').prop('disabled', false);
    $('#copy-room-btn').prop('disabled', false);
    ...
    setTimeout(function() {
        ...
        if (result === 'white_win') { displayMessage = myColor === 'white' ? '🏆 Вы победили!' : '💔 Вы проиграли...'; }
        else if (result === 'black_win') { displayMessage = myColor === 'black' ? '🏆 Вы победили!' : '💔 Вы проиграли...'; }
...
function resetGame() {
    ...
    myColor = null; isMyTurn = false; gameInProgress = false; gameEnded = false;
...
function showNotification(message) {
    ...
    if (message.includes('Победили') || message.includes('Победа')) { $('#turn-indicator').addClass('alert-success'); }
    else if (message.includes('Ничья')) { $('#turn-indicator').addClass('alert-warning'); }
    else if (message.includes('проиграли') || message.includes('Поражение')) { $('#turn-indicator').addClass('alert-danger'); }
    else { $('#turn-indicator').addClass('alert-info'); }
}
...
window.manualStartGame = function() {
    if (ws && ws.readyState === WebSocket.OPEN && roomId) {
        ws.send(JSON.stringify({ type: 'start_game', roomId: roomId }));
        $('#manual-start-btn').hide();
    } else { alert('Нет подключения к серверу или комнате'); }
};

$(document).ready(function() {
    initChessBoard(); connectWebSocket();
    
    $('#new-game-btn').click(function() {
        if (ws && ws.readyState === WebSocket.OPEN) {
            if (roomId) { ws.send(JSON.stringify({type: 'leave_room', roomId: roomId})); roomId = null; }
            resetGame();
            ws.send(JSON.stringify({type: 'find_new_game'}));
            $('#new-game-btn').prop('disabled', true);
            clearHighlights(); $('.game-over-modal').remove();
    ...
    $('#flip-board-btn').click(function() {
        if (board) { var newOrientation = board.orientation() === 'white' ? 'black' : 'white'; board.orientation(newOrientation); clearHighlights(); }
    });
    
    $('#resign-btn').click(function() {
        if (!gameInProgress || gameEnded) return;
        if (confirm('Вы уверены, что хотите сдаться?')) {
            if (ws && ws.readyState === WebSocket.OPEN && roomId) { ws.send(JSON.stringify({type: 'resign', roomId: roomId})); endGame('resign', 'Вы сдались!'); }
        }
    });
    
    $('#offer-draw-btn').click(function() {
        if (!gameInProgress || gameEnded) return;
        if (ws && ws.readyState === WebSocket.OPEN && roomId) { ws.send(JSON.stringify({type: 'offer_draw', roomId: roomId})); }
    });
    
    $('#copy-room-btn').click(function() {
        if (roomId) {
            var link = window.location.origin + window.location.pathname + '?room=' + roomId;
            navigator.clipboard.writeText(link).then(function() { alert('Ссылка скопирована!'); });
        }
    });
    ...
    if (roomParam) {
        var checkConnection = setInterval(function() {
            if (ws && ws.readyState === WebSocket.OPEN) { clearInterval(checkConnection); setTimeout(function() { ws.send(JSON.stringify({ type: 'join_room', roomId: roomParam })); }, 500); }
        }, 100);
    }
});
JS;
